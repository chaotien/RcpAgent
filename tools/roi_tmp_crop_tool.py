...
class SOPSetupTool(tk.Tk):
    ...
    def build_task_list(self):
        ...
        for state in states:
            ...
            # Helper to extract images from target_features
            def extract_images(features, phase):
                for f in features:
                    if f.get("type") == "image" and "path" in f:
                        # Clean up path to just filename if it has dir
    ...
    def on_resize(self, event):
        # Only re-render if it's the canvas frame resizing
    ...
    def render_canvas(self):
        ...
        if c_width <= 1 or c_height <= 1: return # Canvas not ready
        ...
        self.scale_factor = min(scale_w, scale_h) * 0.95 # Leave 5% margin
        ...
        # Save offsets for mouse coord calculation
    ...
    def process_bbox(self):
        ...
        if task.task_type == 'ROI':
            ...
            # Update YAML memory
            if "roi_map" not in self.yaml_data: self.yaml_data["roi_map"] = {}
    ...
    def save_yaml(self):
        ...
        if save_path:
            try:
                # Use sort_keys=False to preserve dict order
